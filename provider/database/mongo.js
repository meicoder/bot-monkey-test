...
class MongoAdapter {
    ...
    init = async () => {
        try {
            ...
            console.log("Error in MongoDB Conection", e);
            return;
        }
    };
    ...
    updateIntent = async (phone, status) => {
        await this.db
            .collection("intents")
            .findOneAndUpdate({ phone }, { $set: { status } }, { sort: { dateAt: -1 }, returnOriginal: false });
    };
    ...
    findIntent = async (phone) => {
        return await this.db.collection("intents").findOne({ phone }, { sort: { dateAt: -1 } });
    };
